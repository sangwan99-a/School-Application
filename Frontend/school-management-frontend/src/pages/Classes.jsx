...
export default function Classes() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editId) {
  await axios.put(`https://school-application-tkmu.onrender.com/api/classes/${editId}`, form)
      } else {
  await axios.post("https://school-application-tkmu.onrender.com/api/classes", form)
      }
  ...
  const handleDelete = async id => {
    ...
    try {
  await axios.delete(`https://school-application-tkmu.onrender.com/api/classes/${id}`)
      fetchClasses()
    } catch (err) {
          ...
          <tbody>
            {classes.map((c) => (
              <tr key={c.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={async () => {
                ...
                setShowModal(true);
                try {
                  const res = await axios.get(`https://school-application-tkmu.onrender.com/api/classes/${c.id}/students`);

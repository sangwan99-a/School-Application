...
function AdmissionApplicationsCard() {
    ...
    async function fetchAdmissionsAndClasses() {
      try {
        const [admRes, classRes] = await Promise.all([
          axios.get("https://school-application-tkmu.onrender.com/api/admissions"),
          axios.get("https://school-application-tkmu.onrender.com/api/classes")
        ]);
  ...
  await axios.post(`https://school-application-tkmu.onrender.com/api/admissions/${adm.id}/approve?classId=${classId}`);
        setActionSuccess("Application approved.");
      } else if (status === "REJECTED") {
        // You need to implement a reject endpoint in backend for this to work
  await axios.post(`https://school-application-tkmu.onrender.com/api/admissions/${adm.id}/reject`);
